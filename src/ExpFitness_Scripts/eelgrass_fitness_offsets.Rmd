...
```{r}
# read in data
setup <- read.csv("data/experiment/GOEEL-eelgrass-Sweden-exp - Setup.csv")
takedown <- read.csv("data/experiment/GO_eelgrass_mesocosm_takedown_data.csv")
head(setup)
head(takedown)
...
write.csv(full_dat, "data/experiment/eelgrass_mesocosm_start_end_complete.csv")
```
...
```{r}
# treatment environmental data
ind_env_trt <- read.csv("EnvDat/Ind_Exp_Env_072125.csv")
pop_env_trt <- read.csv("EnvDat/Pop_Exp_Env_072125.csv")
...
# # At least a 3x3 reciprocal transplant is needed - WE DO NOT HAVE THIS DESIGN
...
### I THINK TO USE THE ABOVE FUNCTION, I NEED TO COERCE MY FITNESS DATA TO FIT WITHIN THE RECIPROCAL GARDEN FRAMEWORK, WHICH DOESN'T REALLY WORK ESPECIALLY AS I DON'T HAVE A 3X3 DESIGN - I ALSO NEED TO SCALE MY FITNESS DATA BY ENVIRONMENTAL DISTANCE BEFORE USING THIS FUNCTION

# need to start by making a function to calculate the fitness offset - WORKING ON THIS!!!!! doesn't work yet
commGardFit <- function(fit_df) {
  fit_frn = fit_df[fit_df$envOrg == "frn",]
  fit_lcl = fit_df[fit_df$envOrg == "lcl",]
  fit_matrix = matrix(nrow = (length(fit_frn$fitness) + length(fit_lcl$fitness)), ncol = 2)
  for (i in (length(fit_frn$fitness) + length(fit_lcl$fitness))) {
    env_dist = fit_df[i,"envdist"]
    fit = fit_lcl[i,3] - mean(fit_frn[,3])
    fit_scale = fit * (1/(0+1)) # using 0 as a placeholder in denominator, will use scaled 0-1 mahalanobis distance once available
    fit_matrix[i,1] = fit_df[i,"population"]
