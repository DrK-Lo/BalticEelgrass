...
exp_pop_dat_trim$coast <- ifelse(exp_pop_dat_trim$Pop %in% c("VIK","VAT","HOG","HOR"), "West", "East")
...
exp_pop_env <- exp_pop_dat_trim[,c("Pop","coast","lat","long","temp_mean_adjust","temp_max","temp_mean_adjust","sal_min","sal_mean_adjust")]
...
exp_env <- exp_pop_env[,c("temp_mean_adjust","temp_max","sal_mean_adjust","sal_min")]
...
full_sea <- merge(indiv_list_full, sea_scaled, by = c("Pop"))
mll_sea <- merge(indiv_list_MLL, sea_scaled, by = c("Pop"))
```
...
mat3 <- as.matrix(full_exp_scaled[,c("temp_scaled", "max_temp_scaled", "sal_scaled", "min_sal_scaled")])
mat4 <- as.matrix(mll_exp_scaled[,c("temp_scaled", "max_temp_scaled", "sal_scaled", "min_sal_scaled")])
mat5 <- as.matrix(full_fut_scaled[,c("temp_scaled", "sal_scaled")])
mat6 <- as.matrix(mll_fut_scaled[,c("temp_scaled", "sal_scaled")])
...
write.env(mat3, paste0("data/EnvDat/full_expEnv_", Sys.Date(), ".env"))
write.env(mat4, paste0("data/EnvDat/mll_expEnv_", Sys.Date(), ".env"))
write.env(mat5, paste0("data/EnvDat/full_futEnv_", Sys.Date(), ".env"))
write.env(mat6, paste0("data/EnvDat/mll_futEnv_", Sys.Date(), ".env"))
```
...
coord_fullexp <- full_exp[,c("long","lat")]
coord_mllexp <- mll_exp[,c("long","lat")]

# generate PCNMs (don't need to do both seascape current and future since these are the same)
pcnm_fullsea <- pcnm(dist(coord_fullsea))
pcnm_mllsea <- pcnm(dist(coord_mllsea))
pcnm_fullexp <- pcnm(dist(coord_fullexp))
