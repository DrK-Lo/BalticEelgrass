...
exp_dat <- read.csv("data/EnvDat/complete_expSites_copernicus2025-08-25.csv")[,-1]
...
Before I can scale any environmental data, I need to also add in the environments I want to predict to (i.e. experimental site of origin, experimental treatments, future environment). I will thus process all of these data as well before coming back to scale and standardize everything. 
...
curr_sea$temp_curr <- as.numeric(curr_sea$temp_curr)
curr_sea$max_temp_curr <- as.numeric(curr_sea$max_temp_curr)
curr_sea$sal_curr <- as.numeric(curr_sea$sal_curr)
curr_sea$min_sal_curr <- as.numeric(curr_sea$min_sal_curr)
fut_sea$temp_8.5 <- as.numeric(fut_sea$temp_8.5)
fut_sea$sal_8.5 <- as.numeric(fut_sea$sal_8.5)

# I can put it together using "Pop"
full_sea <- merge(indiv_list_full, curr_sea, by = c("Pop"))
mll_sea <- merge(indiv_list_MLL, curr_sea, by = c("Pop"))
full_fut <- merge(indiv_list_full, fut_sea, by = c("Pop"))
mll_fut <- merge(indiv_list_MLL, fut_sea, by = c("Pop"))
full_exp <- merge(indiv_list_full, exp_pop_env, by = c("Pop"))
mll_exp <- merge(indiv_list_MLL, exp_pop_env, by = c("Pop"))
```

Perfect, now I have an entry for every individual in the genomic datasets. Finally, I should standardize all of the environmental data.
...
# scale
full_sea_std <- scale(full_sea[,c("temp_curr", "max_temp_curr","sal_curr","min_sal_curr")])
mll_sea_std <- scale(mll_sea[,c("temp_curr", "max_temp_curr","sal_curr","min_sal_curr")])
full_fut_std <- scale(full_fut[,c("temp_8.5","sal_8.5")])
mll_fut_std <- scale(mll_fut[,c("temp_8.5","sal_8.5")])
full_exp_std <- scale(full_exp[,c("temp_mean_adjust", "temp_max","sal_mean_adjust","sal_min")])
mll_exp_std <- scale(mll_exp[,c("temp_mean_adjust", "temp_max","sal_mean_adjust","sal_min")])
colnames(full_sea_std) <- c("temp_sea","max_temp_sea","sal_sea","min_sal_sea")
colnames(mll_sea_std) <- 
colnames(full_exp_std) <- 
colnames(mll_exp_std) <-  c("temp_scaled","max_temp_scaled","sal_scaled","min_sal_scaled")
colnames(full_fut_std) <- colnames(mll_fut_std) <- c("temp_scaled_fut","sal_scaled_fut")

# attach scaled information back to env dfs
full_sea_scaled <- as.data.frame(cbind(full_sea, full_sea_std))
mll_sea_scaled <- as.data.frame(cbind(mll_sea, mll_sea_std))
full_exp_scaled <- as.data.frame(cbind(full_exp, full_exp_std))
mll_exp_scaled <- as.data.frame(cbind(mll_exp, mll_exp_std))
full_fut_scaled <- as.data.frame(cbind(full_fut, full_fut_std))
mll_fut_scaled <- as.data.frame(cbind(mll_fut, mll_fut_std))

# write all of this information out to csv for any future analysis
write.csv(full_sea_scaled, paste0("data/EnvDat/full_seascapeEnv_", Sys.Date(), ".csv"))
write.csv(mll_sea_scaled, paste0("data/EnvDat/mll_seascapeEnv_", Sys.Date(), ".csv"))
write.csv(full_exp_scaled, paste0("data/EnvDat/full_expEnv_", Sys.Date(), ".csv"))
write.csv(mll_exp_scaled, paste0("data/EnvDat/mll_expEnv_", Sys.Date(), ".csv"))
write.csv(full_fut_scaled, paste0("data/EnvDat/full_futEnv_", Sys.Date(), ".csv"))
write.csv(mll_fut_scaled, paste0("data/EnvDat/mll_futEnv_", Sys.Date(), ".csv"))
```
