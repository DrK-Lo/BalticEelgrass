...
###salinity
data <- readMat("SalinityDaily.mat")
...
mean_for_sites <- apply(data$SalinityDaily, MARGIN = 1, mean)
#margin=1 gives mean for each site regardless of day
min_sal_for_sites <- apply(data$SalinityDaily, MARGIN = 1, min)
sal_percentile_10_for_sites <- apply(data$SalinityDaily, MARGIN = 1, function(x) quantile(x, 0.1))
sal_percentile_1_for_sites <- apply(data$SalinityDaily, MARGIN = 1, function(x) quantile(x, 0.01))

###temp
data <- readMat("TempDaily.mat")
#rows are the 39 sites in the same order that I got from you, the columns are the data for the 92 days with 1 June in the first column, and then there are 11 slices, one for each year, where the first slice is year 2011.
...
# Calculate the mean for each site over all years, regardless of the day
T_mean_for_sites <- apply(data$TempDaily, MARGIN = 1, mean)
#margin=1 gives mean for each site regardless of day
T_max_for_sites <- apply(data$TempDaily, MARGIN = 1, max)

# Calculate the 90th percentile for each site
T_percentile_90_for_sites <- apply(data$TempDaily, MARGIN = 1, function(x) quantile(x, 0.9))
T_percentile_99_for_sites <- apply(data$TempDaily, MARGIN = 1, function(x) quantile(x, 0.99))
...
max_temp_for_5_days_for_sites <- apply(data$TempDaily, MARGIN = 1, max_temp_for_5_days)
