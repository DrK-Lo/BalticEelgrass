...
The rdadapt package does not install consistently across platforms, so I am pulling the code for relevant functions directly from the package github page: 
...
trt_expanded <- as.data.frame(rep(as.data.frame(trt_scaled_red), 8))
pop1 <- trt_expanded[,1:4]
pop2 <- trt_expanded[,5:8]
pop3 <- trt_expanded[,9:12]
pop4 <- trt_expanded[,13:16]
pop5 <- trt_expanded[,17:20]
pop6 <- trt_expanded[,21:24]
pop7 <- trt_expanded[,25:28]
pop8 <- trt_expanded[,29:32]
colnames(pop1) <- colnames(pop2) <- colnames(pop3) <- colnames(pop4) <- colnames(pop5) <- colnames(pop6) <- colnames(pop7) <- colnames(pop8) <- c("median_temp_scaled", "max_temp_scaled", "median_sal_scaled", "min_sal_scaled")
trt_expanded <- rbind(pop1, pop2, pop3, pop4, pop5, pop6, pop7, pop8)
trt_expanded <- as.matrix(trt_expanded)
...
# save offsets
write.csv(offsets_rda, paste0("RDA_offsets_exptrt_long_",Sys.Date(),".csv"))
write.csv(offsets_rda_wide, paste0("RDA_offsets_exptrt_wide",Sys.Date(),".csv"))
```
...
```{r}
# plot
ggplot(offsets_rda, aes(x = pop, y = trt, fill = (offset_mll))) +
...
ggplot(offsets_rda, aes(x = pop, y = trt, fill = (offset_full))) +
  geom_tile(color = "white") +
  scale_fill_viridis_c(option = "magma", direction = -1, limits = c(0,4)) +
  theme_classic() +
