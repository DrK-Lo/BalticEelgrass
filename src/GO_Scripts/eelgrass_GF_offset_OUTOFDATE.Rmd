...
####################
# names
names(curr_pop_env_gf_red) <- names(curr_full_env_gf_red) <- names(curr_mll_env_gf_red) <- c("pcnm","temp_scaled","sal_scaled")
...
# names
names(fut_pop_env_gf) <- names(trt_full_env_gf) <- names(trt_mll_env_gf) <- c("pcnm","temp_scaled","sal_scaled")
...
# full af - this is not giving the offset to each treatment
df_full_af_trt <- data.frame() # df
for (i in 1:nb_ind_af_nopcnm){ # calc offset
  df_temp_trt <- data.frame()
  ec_dist <- euclidean(pred_full_af_exporg[i,], pred_full_af_exptrt[1,]) %>% as.data.frame()
  df_full_af_trt <- rbind(df_full_af_trt, ec_dist)
}
colnames(df_full_af_trt) <- "offset_full_af_trt"
# df of pops
pop_df_rep <- pop_list[,-1] %>% slice(rep(1:n(), each = 4))
af_full_offset_complete <- cbind(pop_df_rep, df_full_af) # associate metadata with offset calculation
...
I'm going to visualize offsets first as points on a map before trying to project them as a gradient.
...
map_full_af <- ggplot(data = world) +
  theme_bw()+
  geom_sf(data = world_crop, fill = 'antiquewhite1') +
  geom_point(data = all_offsets_poplvl, aes(x=long, y=lat, 
                                   fill = log(offset_full_af)), 
             shape = 21, size = 4)+
  scale_fill_gradient(low = "green", high = "red", space = "Lab", limits = c(-9.5,-4.4))+
  theme(plot.title = element_text(size = 24), panel.grid.major = element_line(color = "aliceblue"),
        ...
        labs(title = "Genomic Offset of Baltic \nEelgrass (GF, Full, AF)", x = "Longitude", y = "Latitude", fill = "Genomic \nOffset")
(map_full_af)
```
